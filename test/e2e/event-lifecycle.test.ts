...
describe("Event Lifecycle E2E - Happy and Rejection Paths", () => {
    ...
    let setupData: { orgId: number; eventTypeId: number; categoryId: number; hostId: number; approverId: number };
    ...
    beforeAll(async () => {
        ...
        const [approverUser] = await db.insert(schema.user).values({
    ...
    describe.sequential("Denial & Resubmission Path Workflow", () => {
        ...
        it("approver actively denies the event, checking it reverts to draft", async () => {
            ...
            const instances = await db.query.workflowInstance.findMany({ where: eq(schema.workflowInstance.eventId, eventId) });
        ...
        it("host updates and resubmits the event safely", async () => {
            ...
            const activeInstances = await db.query.workflowInstance.findMany({
                where: and(eq(schema.workflowInstance.eventId, eventId), eq(schema.workflowInstance.status, "active"))
            });
