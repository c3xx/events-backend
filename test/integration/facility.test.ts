import { assert, describe, expect, test, beforeAll } from "vitest";
...
import { insertFacilityAssignment } from "@/modules/event/facility-assignments/repository.js";
import { createOrganizerTestSetup, createTestVenue } from "./integration-test-helpers.js";
import { createFacility, changeAvailability } from "@/modules/facility/service.js";
import { findFacilities, findFacilityById, changeAvailability as changeAvailabilityDb } from "@/modules/facility/repository.js";
import { findEventById } from "@/modules/event/repository.js";
import { getEvent, submitEvent, createEvent } from "@/modules/event/service.js";
import { setupWorkflowTestEnvironment, createTestEventBody, getWorkflowForEvent } from "./integration-test-helpers.js";
import { getManagedEntityForOrganization } from "./integration-test-helpers.js";
...
describe("Facility Integration Tests", () => {
	...
	describe("1. Core Facility CRUD & Managed Entity Integrity", () => {
		test.skip("BUG: creates facility securely with isAvailable: false by default... missing managedEntity trigger", async () => {
			...
			const me = await db.query.managedEntity.findFirst({
				where: and(eq(schema.managedEntity.refId, facility.id), eq(schema.managedEntity.managedEntityType, "facility"))
			});
		...
		test("changeAvailability flips isAvailable to true and persists it, then back to false", async () => {
			...
			const recordTrue = await db.query.facility.findFirst({ where: eq(schema.facility.id, facility.id) });
			expect(recordTrue?.isAvailable).toBe(true);

			await changeAvailabilityDb(facility.id, { availability: false });
			const recordFalse = await db.query.facility.findFirst({ where: eq(schema.facility.id, facility.id) });
		...
		test.skip("BUG: changeAvailability on non-existent facility id throws instead of being a no-op", async () => {
			await expect(
				changeAvailabilityDb(999999, { availability: true })
		...
		test("[Edge Case] facility becomes unfetchable via findFacilityById once its facilityType is soft-deleted", async () => {
			const [tempType] = await db.insert(schema.facilityType).values({ name: `TempType-${nanoid()}` }).returning();
			const facility = await createFacility({
				name: `TempFac-${nanoid()}`,
				typeId: tempType!.id,
				association: "event",
			...
			await db.update(schema.facilityType).set({ deletedAt: new Date().toISOString() }).where(eq(schema.facilityType.id, tempType!.id));
		...
		test("soft-deleting the facility itself removes it from both findFacilities and findFacilityById", async () => {
			...
			await db.update(schema.facility).set({ deletedAt: new Date().toISOString() }).where(eq(schema.facility.id, facility.id));
	...
	describe("2. Facility Provider Resolution", () => {
		...
		test("soft-deleted provider row excluded from providers left join", async () => {
			...
			await db.update(schema.facilityProvider)
		...
		test("facility with multiple providers of mixed types (one org, one venue) independently resolve scopes", async () => {
			...
			await db.insert(schema.facilityProvider).values([
				{ facilityId: facility.id, providerEntityType: "organization", providerEntityRefId: validOrgId },
	...
	describe("3. Facility Assignment to Events", () => {
		...
		test("rejects assigning an association: 'venue_allotment' facility when venueAllotmentId is omitted", async () => {
			...
			).rejects.toThrow("This facility can only be associated with a venue");
		});
		...
		test("BUG: [Race Condition] TOCTOU duplicate checker fails atomically under concurrent assignments", async () => {
			...
			const allAssignments = await db.query.eventFacility.findMany({
				where: and(eq(schema.eventFacility.eventId, event.id), eq(schema.eventFacility.facilityId, facility.id))
			});
		...
		test("rejects assignment if venueAllotmentId exists but belongs to a totally different event", async () => {
			...
			const [allotmentB] = await db.insert(schema.venueAllotment).values({
				eventId: eventB.id,
			...
			await expect(
				assignEventFacility(admin, eventA, { facilityId: facility.id, venueAllotmentId: allotmentB!.id })
		...
		test("rejects assignment if the facility lacks a provider natively matching the allotment's physical Venue ID", async () => {
			...
			const diffVenue = await createTestVenue({ venueTypeId: validVenueTypeId, name: `DiffVen-${nanoid()}` });

			const [allotment] = await db.insert(schema.venueAllotment).values({
				eventId: event.id,
				venueId: validVenueId,
				startsAt: new Date(Date.now() + 86400000).toISOString(),
				endsAt: new Date(Date.now() + 172800000).toISOString(),
			...
			await expect(
				assignEventFacility(admin, freshEvent, { facilityId: facRaw.id, venueAllotmentId: allotment!.id })
			).rejects.toThrow("Facility cannot be assigned to this venue allotment, as the venue isn't the provider of this facility");
		});
		
		test("Unassignment strictly enforces ownership permissions and valid ID bindings", async () => {
			const { event, admin } = await createOrganizerTestSetup();
			await expect(
				unassignEventFacility(admin, event, 9999999)
			).rejects.toThrow("Could not find the facility assignment");
		});
	});
	...
	describe("4. Workflow Participation Policy", () => {
		test.skip("BUG: include facility (Org provider) spans directly to Target Groups (Crashes due to missing GROUP BY in findFacilityManagedEntities)", async () => {
			...
			// Assign it
			await assignEventFacility({ id: setup.hostUser.id, type: "end_user" }, fullEventScope, { facilityId: facility.id });
			...
			// Expect that the target groups dynamically map an ID bound to the provider Org!
			const targetOrgFound = targetGroups.some(tg => tg.stepRole.step.instance.eventId === createdEvent.id);
			expect(targetOrgFound).toBeTruthy();
			...
			const orgME = await db.query.managedEntity.findFirst({ where: and(eq(schema.managedEntity.refId, validOrgId), eq(schema.managedEntity.managedEntityType, "organization")) });
			assert(orgME);
		...
		test.skip("BUG: exclude facility entirely omits all target groups natively (Crashes due to missing GROUP BY in findFacilityManagedEntities)", async () => {
			...
			const createdEvent = await createEvent(
				{ id: setup.hostUser.id, type: "end_user" },
				createTestEventBody({ organizationId: setup.eventOrg.id, typeId: setup.eventType.id, categoryId: setup.category.id }),
			);
			const fullEvent = await findEventById(createdEvent.id);
			assert(fullEvent);
			const fullEventScope = await getEvent(fullEvent);
			...
			const pointsToProviderOrg = orgME ? targetGroups.some(tg => tg.managedEntityId === orgME.id) : false;
		...
		test.skip("BUG: Empty Target Generation - facility with strictly zero mapped providers succeeds submission but produces dangerous empty target (Crashes due to missing GROUP BY)", async () => {
			...
			const submissionPromise = submitEvent({ id: setup.hostUser.id, type: "end_user" }, await getEvent(finalEvent));
	...
	describe("5. Overlap Policy", () => {
		test("Reject exclusive assignment to Event B natively only if Event A is approved", async () => {
			...
			await db.update(schema.event).set({ status: "approved" }).where(eq(schema.event.id, eventA.id));
		...
		test("Openly ALLOW assigning exclusive facility to Event B if Event A is still in a draft or pending state", async () => {
			...
			const assignB = await assignEventFacility(admin, await getEvent(freshDbEventB), { facilityId: facility.id });
