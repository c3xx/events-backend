...
import * as approvalRepository from "@/modules/me/approval-assignments/repository.js";
...
export async function getManagedEntityForOrganization(organizationId: number) {
    ...
    if (!managedentity)
        throw new Error("Failed to create or find managed entity");
    return managedentity;
}

export async function createTestEventType(data: {
    name?: string;
...
export async function grantPermissionToRole(
    ...
    if (!perm)
        throw new Error(`Failed to ensure permission ${permissionCode} exists`);
...
export async function createTestWorkflowStepRole(data: {
    ...
    const [stepRole] = await db
        ...
        .values({
            stepId: data.stepId,
            roleId: data.roleId,
            targetGroupApprovalCriteria:
                data.targetGroupApprovalCriteria ?? "all",
        })
        .returning();
...
export async function allowParentType(
    childTypeId: number,
    parentTypeId: number,
) {
    await db
...
export async function createBasicEventSetup() {
    ...
    if (!hostME)
        throw new Error("Expected managed entity for host organization");
    ...
    await grantPermissionToRole(
        endUserRole.id,
        "event:manage" as PermissionCode,
    );
...
export async function createOrganizerTestSetup() {
    ...
    await grantPermissionToRole(
        mockRole.id,
        "event_organizer_invitation:respond" as PermissionCode,
    );
    await grantPermissionToRole(
        mockRole.id,
        "event_organizer:manage" as PermissionCode,
    );
...
export function createTestEventBody(overrides: {
    ...
    return {
        ...
        expectedParticipants:
            overrides.expectedParticipants !== undefined
                ? overrides.expectedParticipants
                : 10,
        requestDetails:
            overrides.requestDetails ?? "Testing host auto-creation",
        startsAt:
            overrides.startsAt ?? new Date(Date.now() + 86400000).toISOString(),
        endsAt:
            overrides.endsAt ?? new Date(Date.now() + 172800000).toISOString(),
    };
}
...
export async function getWorkflowForEvent(eventId: number) {
    ...
    const orderedSteps = orderWorkflowSteps(
        instance.steps,
        instance.initialStepId,
    );
...
export async function getWorkflowAssignmentForUser(
    userId: number,
    eventId: number,
) {
    ...
    const assignment = assignments.find(
        (a) =>
            a.userRole.userId === userId &&
            a.targetGroup.stepRole.step.instance.eventId === eventId,
    );
...
export async function approveCurrentAssignment(
