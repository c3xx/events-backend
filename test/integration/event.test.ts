...
import { getWorkflowAssignmentForUser } from "./integration-test-helpers.js";
...
describe("Event Integration Tests", () => {
    describe("event lifecycle", () => {
        test("create event creates host organizer", async () => {
            const { endUser, hostOrg, eventType, category } =
                await createBasicEventSetup();
        ...
        test("createEventSchema validation for expectedParticipants", () => {
            ...
            expect(zeroIssue.message).toBe(
                "Expected participants must be positive",
            );
            ...
            expect(negativeIssue.message).toBe(
                "Expected participants must be positive",
            );
            ...
            expect(floatIssue.message).toBe(
                "Invalid expected participants count",
            );
            ...
            expect(nanIssue.message).toBe(
                "Invalid expected participants count",
            );
        });

        test("create event with inactive event type should fail", async () => {
            const { endUser, hostOrg, category, eventType } =
                await createBasicEventSetup();
        ...
        test("create event with invalid organization should fail", async () => {
            const { endUser, eventType, category } =
                await createBasicEventSetup();
        ...
        test("create event with unauthorized user should fail", async () => {
            const { hostOrg, eventType, category } =
                await createBasicEventSetup();
...
describe("Workflow Instance Management", () => {
    test.skip("Submitting event again that has exisiting workflow with active status is denied", async () => {
        const { admin, hostOrg, eventType, category, fullEvent } =
            await createAndSubmitBasicEvent();
        ...
        await expect(
            submitEvent({ id: admin.id, type: "admin" }, fullEvent),
        ).rejects.toThrow();
        await expect(
            submitEvent({ id: admin.id, type: "admin" }, fullEvent2),
        ).rejects.toThrow();
    });
    ...
    test("Submit event that does not exist should fail", async () => {
        ...
        await expect(
            submitEvent({ id: admin.id, type: "admin" }, {
                id: 9999999,
            } as any),
        ).rejects.toThrow();
    });
    ...
    test.skip("Submit event with inactive event type should fail", async () => {
        const { admin, hostOrg, eventType, category } =
            await createBasicEventSetup();
        const [inactiveEventType] = await db
            .insert(schema.eventType)
            .values({
        ...
        await expect(
            submitEvent({ id: admin.id, type: "admin" }, updatedFullEvent),
        ).rejects.toThrow();
    });
    ...
    test("Submitting an event with an existing workflow instance that is rejected should create a new workflow instance", async () => {
        ...
        await expect(
            submitEvent({ id: admin.id, type: "admin" }, eventFound),
    ...
    test("Submitting an event creates a workflow instance that exactly matches the workflow template", async () => {
        ...
        const orderedTemplateSteps = orderWorkflowSteps(
            template.steps,
        ...
        const instanceSteps = await db.query.workflowInstanceStep.findMany({
            where: eq(
                schema.workflowInstanceStep.instanceId,
                workflowInstance.id,
            ),
            with: {
                roles: true,
        ...
        function orderInstanceSteps(
            steps: typeof instanceSteps,
        ...
        const orderedInstanceSteps = orderInstanceSteps(
            instanceSteps,
            workflowInstance.initialStepId,
        );
        ...
        for (let i = 0; i < orderedTemplateSteps.length; i++) {
            ...
            expect(workflowInstance.initialStepId).toBe(
                orderedInstanceSteps[0]!.id,
            );

            for (let i = 0; i < orderedInstanceSteps.length - 1; i++) {
                expect(orderedInstanceSteps[i]!.nextStepId).toBe(
                    orderedInstanceSteps[i + 1]!.id,
                );
            }

            expect(
                orderedInstanceSteps[orderedInstanceSteps.length - 1]!
                    .nextStepId,
            ).toBeNull();

            // Number of roles copied
            expect(instanceStep.roles).toHaveLength(
                templateStep.stepRoles.length,
            );
            ...
            if (i === orderedInstanceSteps.length - 1) {
                ...
                expect(instanceStep.nextStepId).toBe(
                    orderedInstanceSteps[i + 1]!.id,
                );
            }
        }
    ...
    test("Submitting two events simultaneously should succeed", async () => {
        ...
        const promise1 = submitEvent(
            { id: admin.id, type: "admin" },
            fullEvent1,
        );

        const promise2 = submitEvent(
            { id: admin.id, type: "admin" },
            fullEvent2,
        );
...
describe("Workflow Approval Execution", () => {
    test("Workflow and step statuses are updated correctly as approvals progress", async () => {
        ...
        // First HOD
        const hod1Assignment = await getWorkflowAssignmentForUser(
            approvers.hod1.id,
            fullEvent.id,
        );
        ...
        // Second HOD
        const hod2Assignment = await getWorkflowAssignmentForUser(
            approvers.hod2.id,
            fullEvent.id,
        );
    ...
    test("ANY approval skips remaining assignments", async () => {
        ...
        const assignments =
            workflow.steps[2]!.roles[0]!.targetGroups[0]!.assignments;

        const updatedPrincipal1 = assignments.find(
            (a) => a.id === principal1Assignment.id,
        );

        const updatedPrincipal2 = assignments.find(
            (a) => a.id === principal2Assignment.id,
        );
    ...
    test("Denying an assignment aborts the workflow", async () => {
        ...
        const assignments =
            workflow.steps[0]!.roles[0]!.targetGroups[0]!.assignments;
        console.log(advisorAssignment);

        console.log(workflow.steps[0]!.roles[0]!.targetGroups[0]!.assignments);
        const advisor = assignments.find(
            (a) => a.userRoleId === advisorAssignment.userRoleId,
        );
    ...
    test("ALL approval requires every approver before advancing", async () => {
        ...
        const finalHod1Assignment = await getWorkflowAssignmentForUser(
            approvers.hod1.id,
            fullEvent.id,
        );

        const finalHod2Assignment = await getWorkflowAssignmentForUser(
            approvers.hod2.id,
            fullEvent.id,
        );
